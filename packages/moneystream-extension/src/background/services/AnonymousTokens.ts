...
import * as anonymousTokens from '@moneystream/anonymous-tokens'
...
export class AnonymousTokens extends anonymousTokens.AnonymousTokens {
  constructor(
    @inject(tokens.MoneystreamDomain) moneystreamHost: string,
    @inject(Storage) storage: Storage,
    @logger('AnonymousTokens') debug: Logger
  ) {
    super({
      redeemerUrl: moneystreamHost + '/redeemer',
      signerUrl: moneystreamHost + '/issuer',
