//modeled after ilp DataAndMoneyStream
//but this one is for Bitcoin
//represents a stream of bitcoin transaction
//like what a bitcoin wallet can emit
 ...
 */
@injectable()
export class BitcoinStream extends Duplex {
  readonly id: number
  ...
  setSendMax (limit: LongValue): void {
    ...
    if (this._totalSent.greaterThan(sendMax)) {
      this.log('cannot set sendMax to %s because we have already sent: %s', sendMax, this._totalSent)
      throw new Error(`Cannot set sendMax lower than the totalSent`)
    }
    this.log(`setting sendMax to ${sendMax}`)
  ...
  setReceiveMax (limit: LongValue): void {
    ...
    if (this._totalReceived.greaterThan(receiveMax)) {
      this.log('cannot set receiveMax to %s because we have already received: %s', receiveMax, this._totalReceived)
    ...
    await new Promise((resolve, reject) => {
      ...
      function endHandler () {
        ...
        if ((self._totalSent.greaterThanOrEqual(limit))) {
          ...
          self.log('Stream was closed before the desired amount was sent (target: %s, totalSent: %s)', limit, self._totalSent)
   ...
   */
   _addToIncoming (amount: Long /*, prepare: IlpPrepare*/): void {
     ...
     this.log(`received ${amount} (totalReceived: ${this._totalReceived})`)
  ...
  _holdOutgoing (holdId: string, maxAmount?: Long): Long {
    ...
    if (amountToHold.greaterThan(0)) {
      ...
      this.log('holding outgoing balance. holdId: %s, amount: %s', holdId, amountToHold)
    }
    return amountToHold
  }
  ...
  _final (callback: (...args: any[]) => void): void {
    this.log('stream is closing')
    const finish = (err?: Error) => {
      if (err) {
        this.log('error waiting for money to be sent:', err)
      }
    ...
    if (this._remoteSentEnd || this._sendMax.lessThanOrEqual(this._totalSent)) {
      ...
      this.log('waiting to finish sending money before ending stream')
  ...
  _write (chunk: Buffer, encoding: string, callback: (...args: any[]) => void): void {
    this.log('%d bytes written to the outgoing data queue', chunk.length)
  ...
  _writev (chunks: { chunk: Buffer, encoding: string }[], callback: (...args: any[]) => void): void {
    for (let i = 0; i < chunks.length; i++) {
      ...
      this.log('%d bytes written to the outgoing data queue', chunk.chunk.length)
  ...
  _read (size: number): void {
    ...
    if (!data) {
      ...
      //TODO: there is no super.readableFlowing?
  ...
  _getAvailableDataToSend (size: number): { data: Buffer | undefined, offset: number } {
    ...
    if (data && data.length > 0) {
      ...
      this.log('%d bytes taken from the outgoing data queue', data.length)
    }
    return { data, offset }
  }
  ...
  _resendOutgoingData (data: Buffer, offset: number) {
    this.log('re-queuing %d bytes of data starting at offset %d', data.length, offset)
  ...
  protected safeEmit (event: string, ...args: any[]) {
    try {
      //TODO: had to hack up args to cast it into something
      //that emit.apply likes
