import {
  ...
  StorableBlindToken,
  verifyProof
} from '@moneystream/privacypass-sjcl'
...
export interface AnonymousTokensOptions {
  // The `protocol://host` of the moneystream services.
  redeemerUrl: string
  signerUrl: string
  store: TokenStore
...
export class AnonymousTokens {
  ...
  async getToken(moneystreamAuthToken: string): Promise<string> {
    // When there is only 1 token left, fetch some more in the background.
    if (this.storedTokenCount === 1) {
      this.populateTokens(moneystreamAuthToken)
    }
    ...
    while (true) {
      ...
      if (!token) {
        await this.populateTokens(moneystreamAuthToken)
        continue
      }
  ...
  private async _signToken(
    moneystreamAuthToken: string,
    request: string
  ): Promise<IssueResponse> {
    const signRes = await portableFetch(this.signerUrl + '/issue', {
      method: 'POST',
      headers: {
        authorization: `Bearer ${moneystreamAuthToken}`,
  ...
  private async populateTokens(moneystreamAuthToken: string): Promise<void> {
    ...
    this._populateTokensPromise = this._populateTokensNow(moneystreamAuthToken)
    try {
  ...
  private async _populateTokensNow(
    moneystreamAuthToken: string
  ): Promise<void> {
    ...
    const signPromise = this._signToken(
      moneystreamAuthToken,
      issueRequest
    ).then(async (issueResp: IssueResponse) => {
