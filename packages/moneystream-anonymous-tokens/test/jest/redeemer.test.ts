...
;(TOKENS_SERVICE_TEST ? describe : describe.skip)(
  ...
  () => {
    ...
    let moneystreamAuthToken: string

    beforeEach(() => {
      moneystreamAuthToken = jwt.sign(
        {
          agg: 1000000000000,
          userId: 'abcdefg'
        },
    ...
    it('should successfully get tokens issued & redeemed', async () => {
      await tokens['populateTokens'](moneystreamAuthToken)

      const btpToken = await tokens.getToken(moneystreamAuthToken)
    ...
    it('should redeem twice', async () => {
      ...
      const btpToken = await tokens.getToken(moneystreamAuthToken)
      const btpToken2 = await tokens.getToken(moneystreamAuthToken)
      const decoded = jwt.verify(btpToken, BTP_SECRET) as any
